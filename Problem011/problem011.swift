/*
 ...
 格子内の最大の積 (Swift)
 20x20のグリッドの中で、上下左右斜めいずれかの方向で連続する4つの数字の積の最大値を求める
*/
...
// MARK: - 愚直実装
func solveBruteForce(_ numRows: Int, _ numCols: Int, _ sequenceLength: Int) -> Int {
    var maxProduct = 0
    
    for row in 0..<numRows {
        for col in 0..<numCols {
            let canGoRight = col + sequenceLength <= numCols
            let canGoDown = row + sequenceLength <= numRows
            let canGoDiagonalRight = canGoDown && canGoRight
            let canGoDiagonalLeft = canGoDown && col - sequenceLength + 1 >= 0
            
            // 右方向
            if canGoRight {
                let product = (0..<sequenceLength).map { grid[row][col + $0] }.reduce(1, *)
                maxProduct = max(maxProduct, product)
            }
            
            // 下方向  
            if canGoDown {
                let product = (0..<sequenceLength).map { grid[row + $0][col] }.reduce(1, *)
                maxProduct = max(maxProduct, product)
            }
            
            // 右下斜め
            if canGoDiagonalRight {
                let product = (0..<sequenceLength).map { grid[row + $0][col + $0] }.reduce(1, *)
                maxProduct = max(maxProduct, product)
            }
