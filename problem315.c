 ...
 * Source: http://odz.sakura.ne.jp/projecteuler/index.php?cmd=read&page=Problem%20315
 * 
 * Problem 315 「デジタルルート時計」  †
 * Sam と Max は, ２個のデジタル時計を「デジタルルート（数字根）時計」（補注参照）に作り変えるよう依頼されている.
 ...
 * 数 "1" は右上と右下の縦セグメントでできており, 数 "4" は中の横セグメントと, 左上, 右上, 右下の縦セグメントからできている. 数
 * "8" はすべてのセグメントが点灯する.
 * 時計はセグメントを点灯/消灯させるときに限りエネルギーを消費する.
 * "2" を点灯させるには 5 回の遷移を要する. "7" は 4 回だけ遷移を要する.
 * Sam と Max は２個の異なる時計を作る.
 * Sam の時計は 137 のような数を与えられると, "137" を表示し, パネルを消灯してから次の数（"11"）を点灯し,
 * 再び消灯してそして最終的に最後の数（"2"）を点灯, しばらくして消灯する.
 ...
 * ２つの時計に A = 107 から B = 2×107 の間の全ての素数が与えられる.
 * Sam の時計で必要な遷移の総数と Max の時計で必要な遷移の総数の差を求めよ.
 * 補注：数字根（すうじこん、英: digital
 * root）とは、正の整数値の各位の和（数字和）を求め、結果の数字和を求め、という操作を繰り返し、最終的に得られる 1 桁の数を指す。
 */
...
int main() {
    printf("Project Euler - Problem 315\n");
