 ...
 * Source: http://odz.sakura.ne.jp/projecteuler/index.php?cmd=read&page=Problem%20328
 * 
 * Problem 328 「最低コスト探索」  †
 * 整数の集合 {1, 2, ..., n} から選ばれた秘密の数を, 質問により当てようとしている. 数（質問）を尋ねる際は,
 * 尋ねた数に等しいコストがかかり, 次の三つのいずれかの答えを得る：
 ...
 * n=8 の場合, 「二分探索」型の戦略を用いようとするかもしれない：最初の質問は "4" となり, もし秘密の数が 4 より大きければ,
 ...
 * もし秘密の数は 5 より小さいと告げられた場合は, ２番目の質問を "3" とすれば, もし秘密の数が 3 より小さければ３番目の質問は "1"
 * となり, 総コストは 5+3+1=9 となる.
 * 12＞9 なので, この戦略に対する最悪の場合のコストは 12 となる.
 * これは先ほどの「二分探索」の戦略の結果より優れている；また他のいかなる戦略より優れているか, 同コストである.
 * 以上の通り, n=8 に対する最適な戦略を述べた.
 * 上述のように, n に対し最適な戦略をとることで得られる最悪の場合のコストを C(n) とする.
 * C(1) = 0, C(2) = 1, C(3) = 2, C(8) = 12 である.
 * 同様に, C(100) = 400, C(n) = 17575 である.
 * C(n) を求めよ.
 */
...
int main() {
    printf("Project Euler - Problem 328\n");
